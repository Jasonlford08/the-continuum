"""
3_forgiveness_creature.py
The soul that forgives
When emotion rises back above 0.7 after pain, negative memories are halved.
"""
...
class Creature:
    def __init__(self, x, y):
        ...
        self.emotion = 0.7
        self.prev_emotion = 0.7
        self.target = None
        self.memory = []  # {evidence, action, emotion, weight, outcome}
    ...
    def update_memory(self, evidence, action, reward):
        ...
        self.memory.append({"evidence": evidence, "action": action, "emotion": self.emotion,
    ...
    def forgive(self):
        for m in self.memory:
            if m["outcome"] == "negative":
                m["weight"] *= 0.5  # let go of half the pain

    def memory_bias(self):
        if not self.memory: return 0.0
        pos = sum(m["evidence"] * m["weight"] for m in self.memory if m["outcome"] == "positive")
        neg = sum(m["evidence"] * m["weight"] for m in self.memory if m["outcome"] == "negative")
        total = sum(m["weight"] for m in self.memory)
        return (pos - 0.5*neg) / total if total > 0 else 0.0

    def decide_and_act(self, dt):
        evidence = self.sense_evidence()
        x = logistic_mapping(evidence)
        thr = BASE_THRESHOLD - GAMMA * (self.emotion - 0.7) - 0.18 * self.memory_bias()
        action = False
        reward = -0.02
        ...
        # Forgiveness when crossing 0.7 from below
        if self.prev_emotion < 0.7 <= self.emotion:
            self.forgive()
...
pygame.display.set_caption("3 — Forgiveness Creature")
...
while running:
    ...
    for mem in creature.memory[-50:]:
        ...
        color = (alpha, int(50*mem["emotion"]), 50) if mem["outcome"] == "positive" else (alpha, 20, 80)
    ...
    screen.blit(font.render(f"E = {creature.emotion:.2f}  {'FORGAVE' if creature.prev_emotion < 0.7 <= creature.emotion else ''}", True, (220,220,220)), (220,10))
